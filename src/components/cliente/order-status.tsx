...
import {clearCompletedCustomerFlow} from "@/lib/orders";
...
export function ClientOrderStatus({id,token}:{id:string;token:string}){
  ...
  const loadOrder=useCallback(async()=>{try{const response=await fetch(`/api/orders/${id}?token=${encodeURIComponent(token)}`,{cache:"no-store"}),body=await response.json();if(!response.ok)throw new Error(body.error??"Pedido não encontrado.");setOrder(body);setReviewed(Boolean(body.reviewed))}catch(reason){setError(reason instanceof Error?reason.message:"Pedido não encontrado.")}},[id,token]);
